...
阶段是可独立交付和验收的业务里程碑，不是任务清单。大型设计通常控制在 3 到 6 个阶段；超过
6 个阶段时，先检查是否存在过度拆分，并在总览说明每个额外阶段必须独立存在的原因。以下工作应
合并到同一阶段：

- 共同完成一个用户流程，拆开后没有可用行为；
- 共享同一组 API、Schema 或事务契约，必须一起评审；
- 验收时需要重复运行基本相同的测试、构建或环境准备；
- 只是同一能力的 entity、svc、ctl、前端或测试技术分层。

只有存在明确的部署顺序、兼容窗口、高风险数据变更、外部依赖门禁，或能单独交付并减少后续风险
时才拆成独立阶段。阶段内部的实现步骤使用清单或实现泳道表达，不继续增加阶段编号。
...
- 标明阶段依赖和并行组；同一串行依赖链最多一个阶段为 `进行中`，同一并行组可有多个阶段同时
  `进行中`；
- 记录最近通过验收的阶段及其具体验证证据；
...
大型前后端功能必须在最终阶段安排联合验证泳道，写明后端启动命令、前端 build/dev 命令、测试
数据或初始化前置条件、API 契约检查和可观察端到端流程。各泳道单独编译通过不能替代联合验证，
但联合验证本身通常不需要再拆成多个阶段。

### 阶段顺序和后续扩展

除非总览明确证明阶段相互独立，否则实现按编号串行推进。可并行阶段必须在总览中属于同一并行组，
并同时满足：共同前置阶段已验收；不依赖对方未提交的 API、Schema 或生成物；不修改同一共享文件；
数据库、端口、全局注册表和测试资源不会互相干扰。并行组内可以同时有多个 `进行中` 阶段，并分别
验收；依赖该组的后续阶段必须等待组内必需阶段全部完成。

不要为了制造并行机会而拆细阶段。同一业务闭环的 entity、svc、ctl、前端和测试仍放在一个阶段，
通过实现泳道并行。只有本身能够独立交付、独立验收的业务里程碑才使用并行阶段。共享入口由一个
集成负责人在并行结果就绪后串行合并。每个阶段结束时保持 workspace 可构建。

阶段验收运行能证明本阶段契约成立的最小充分检查：优先目标测试、受影响 crate check 和必要的
静态契约检查。不要在每个阶段重复完整 workspace、全前端构建、真实数据库和端到端流程；这些
高成本验证集中到最终验收阶段，除非当前阶段修改了对应共享边界或后续阶段必须依赖其结果。
...
❌ 按 entity、svc、ctl、前端和测试机械拆成多个阶段
❌ 为了并行而拆阶段，实际仍共享 API、Schema 或聚合入口
❌ 每个阶段重复运行相同的全 workspace 和端到端验证
❌ 只验证各端独立 build，不设计前后端联合验收
✅ 总览维护全局不变量和阶段进度，阶段文档维护可验收规格
✅ 同一业务闭环放在一个阶段，内部任务用清单或实现泳道表达
✅ 独立业务里程碑组成并行组，共享入口由集成负责人串行收口
...
docs/dev/design/membership-migration/01-contract-and-data.md
docs/dev/design/membership-migration/02-business-and-api.md
docs/dev/design/membership-migration/03-frontends-and-verification.md
```

总览固定用户契约、全局不变量、阶段依赖/并行组、最近验收证据和恢复点。第一阶段合并契约清单、entity、
业务 crate alias、表字段备注、联合索引、`install.rs` 与存量数据迁移；第二阶段在一个业务闭环内
完成 `SeaTrans`、幂等、跨数据源补偿、`ApiRouter` 和权限；第三阶段迁移 PC/小程序并使用 SQLite、
目标 crate 检查、前端 typecheck/test/build 和登录后核心流程完成联合验收。entity、svc、ctl 和
测试属于阶段内部实现项，不再各占一个阶段。如果 PC 与小程序只依赖第二阶段已接受的 API 契约、
没有共享写文件和构建资源，可在第三阶段内使用两个实现泳道并行；如果它们本身可独立交付，也可在
总览中定义为同一并行组的两个阶段，最终联合验收等待二者完成。
