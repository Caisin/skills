...
name: design
description: |
  Use when 为当前仓库新增或修改非平凡功能的设计文档，需要明确公共契约、数据一致性、兼容性与验证计划。
  ...
  触发词：设计文档、design doc、架构设计、API 设计、Schema 设计、迁移方案、兼容性方案
---

# kx-rs Design Documents

设计文档位于 `docs/dev/design/`，用于约束非平凡变更的可观察行为和长期边界。
...
1. 从模板创建 `docs/dev/design/<feature-name>.md`，保留有意义的章节顺序。
