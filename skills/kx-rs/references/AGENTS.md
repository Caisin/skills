...
pub fn apis() -> ApiRouter {
    ApiRouter::new()
        .nest("/xxx", ctl::xxx_ctl::apis())
}
```

### 8.5 控制器模板（ctl/xxx_ctl.rs）
...
async fn get_by_id(Path(id): Path<i64>) -> Result<R<XxxTable>, AxumErr> {
    let c = &mut SeaOrms::xxx().await?;
    let item = XxxTable::get(c, id).await?;
    Ok(item.into())
}
...
pub async fn list_by_uid(uid: i64) -> Result<Vec<XxxTable>> {
    let c = &mut SeaOrms::xxx().await?;
    XxxTable::sel()
        .uid_eq(uid)
        .is_del_eq(false)
...
R::with_data(code, data)       // 自定义 code + data
R::with_msg(code, "msg")       // 自定义 code + msg
...
// 自动转换：anyhow::Error -> AxumErr（通过 From trait）
// Handler 签名：async fn xxx() -> Result<R<T>, AxumErr>
// 业务代码中直接用 ? 传播 anyhow 错误即可
```

---

## 10) crud_api! 宏（快速生成标准 CRUD 控制器）

```rust
use kx_axum::crud_api;

// 自动生成 all/page/save/get/del 方法
