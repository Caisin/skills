...
EntitySelect::<field 条件>/asc_<field>/desc_<field>/limit/offset/distinct
...
动态复合条件、动态排序、JOIN/聚合、CAS 影响行数校验等不适合生成 Query 的场景，才直接使用
`Entity`、`COLUMN` 与 SeaORM builder。宏生成能力是便利 API，不改变 insert/upsert/CAS 的业务语义。
